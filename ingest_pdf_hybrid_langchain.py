...
def parse_section_v2(
    ...
    lines = items  # ✅ biar idx/lines valid (kamu pakai pola ini di beberapa helper)
    guessed_unit = default_rm_unit or "kg/km"

    # =========================
    # helpers kecil yang kamu butuhkan
    # =========================
    def _next_numeric_spec(lines_: List[str], idx_: int, max_hop: int = 4) -> Optional[Tuple[int, str]]:
        """Cari baris berikutnya yang isinya angka murni (mis: '3,0')"""
        for k in range(idx_ + 1, min(len(lines_), idx_ + 1 + max_hop)):
            t = norm(lines_[k])
    ...
    def add_rm(ttype: str, qty: float, unit: Optional[str] = None):
        ...
        # ✅ normalisasi khusus Binder Tape
    ...
    def strip_rm_from_text(s: Optional[str]) -> Optional[str]:
        """Hapus 'TYPE + QTY' yang kebawa di specified"""
        if not s:
            return s
        out = s
    ...
    MATERIAL_HINTS2 = [
        ...
        "tinned","annealed","wbt","non-conductive","nonconductive"
    ]

    # =========================
    # 1) PRE-EXTRACT raw materials (global scan)
    # =========================
    rm_unit_pat = re.compile(
        r"(?i)\b([A-Za-z][A-Za-z0-9\.\(\)/% \-]{2,120}?)\s+(\d+(?:[.,]\d+)?)\s*(kg\s*/\s*km|kg/km)\b"
    )
    for m in rm_unit_pat.finditer(section_text or ""):
    ...
    for m in rm_area_pat.finditer(section_text or ""):
        ttype = norm(m.group(1))
        qty = _parse_qty_float(m.group(2))
        if ttype and qty is not None and _looks_like_raw_material(ttype):
            add_rm(ttype, qty, "m²")  # ✅ unit area

    raw_materials = dedup_rm(raw_materials)

    # =========================
    # 2) PARSE per item -> technical_specs + tangkap RM di ekor
    # =========================
    pending_rm_suffix = None
    has_copper_tape = (
        re.search(r"(?i)\bcopper\b", section_text or "") is not None
        and re.search(r"(?i)\btape\b", section_text or "") is not None
    )
    ...
    for idx, raw in enumerate(items):
        ...
        # ---- (A) tangkap kasus Copper Tape pecah ----
        if re.fullmatch(r"(?i)copper\W*tape", r):
            pending_rm_suffix = "Copper Tape"
            continue
        if re.fullmatch(r"(?i)copper", r):
            pending_rm_suffix = "Copper"
            continue
        if pending_rm_suffix == "Copper" and re.fullmatch(r"(?i)tape", r):
            pending_rm_suffix = "Copper Tape"
            continue

        # ---- (B) "Tinned Annealed 3799,1" ----
        ...
        # ---- (C) trailing raw material di akhir baris ----
        ...
        # ---- skip marking lines masuk technical_specs ----
        if re.search(r"(?i)\bmarking\b", r) and (
            "marking of cable" in r.lower() or "cable marking" in r.lower()
        ):
            continue

        # ---- SPECIAL: n/denier row ----
        ...
        # ---- split desc/unit/specified normal ----
        unit = find_unit(r)
        desc = r
        specified = None
        ...
        # ---- Nom thickness: ambil dari baris berikut kalau kepisah ----
        ...
        # ---- extract RM dari specified tail (covering case) ----
        if specified:
    ...
    # ✅ FIX unit WBT
    for rm in raw_materials:
        if "wbt" in (rm.get("type") or "").lower():
            rm["unit"] = "m²"
...
def safe_section_error(sec_name: str, err: Exception, raw: str, max_raw: int = 5000) -> Dict:
    """
    Return objek aman untuk section yang gagal diparse.
    raw_text dipotong supaya ukuran dokumen Mongo tidak meledak.
    """
    raw = raw or ""
    return {
        "error": f"{type(err).__name__}: {str(err)}",
        "raw_text": raw[:max_raw],
        "raw_len": len(raw),
        ...
        try:
            ...
            for cable_title, cable_text in cable_blocks:
                ...
                for sec_name, sec_body in ordered_sections:
                    ...
                    # ✅ ERROR ISOLATION: apapun yang terjadi, jangan bikin PDF fail
                    try:
                        # -------------------------
                        # FINAL TEST
                        # -------------------------
                        if sec_name == "Final Test":
                            sec_body2 = re.split(r"(?i)\b11\s+Packing\b|\bPacking\b", sec_body, maxsplit=1)[0].strip()
                            cable_section["section"]["final_test"] = parse_final_test_v2(sec_body2)
                            continue
                        ...
                        # ✅ simpan aman, jangan stop PDF ini
                        cable_section["section"][s_key] = safe_section_error(sec_name, e, sec_body)
                    ...
                    if isinstance(obj, dict) and obj.get("error"):
                        # section ini gagal diparse, jangan diproses cleanup lanjut
                        continue
            ...
            def collect_section_errors(section_obj: Dict) -> List[Dict]:
                errs = []

                # mode multi-cable
                if isinstance(section_obj, dict) and "cables" in section_obj:
                    for c in section_obj.get("cables", []):
                        sec = (c or {}).get("section", {})
                        for k, v in (sec or {}).items():
                            if isinstance(v, dict) and v.get("error"):
                                errs.append({"cable": c.get("title", "-"), "section": k, "error": v.get("error")})
                else:
                    for k, v in (section_obj or {}).items():
                        if isinstance(v, dict) and v.get("error"):
                            errs.append({"section": k, "error": v.get("error")})

                return errs


            parse_errors = collect_section_errors(section_obj)
            ...
            doc = {
                ...
                "parse_errors": parse_errors,          # ✅ baru
                "parse_error_count": len(parse_errors),# ✅ baru
