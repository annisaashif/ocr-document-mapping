...
def split_type_qty_last_number(s: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    "Wooden Drum 120 1" -> ("Wooden Drum 120", "1")  (ambil angka TERAKHIR)
    """
    s = norm(s)
    if not s:
        return None, None
...
def clean_desc(desc: str) -> str:
    d = norm(desc)
    # hilangkan bullet "- " tapi pertahankan isi
    d = re.sub(r"^\-\s*", "", d)
    return d
...
def extract_marking_from_page_text(pdf_path: str) -> Optional[str]:
    # marking paling stabil via text page
    try:
                ...
                if "ref" in norm(df.iat[r, c]).lower():
                    ...
                    for s in specs:
                        ...
                        if "raw material" in s.lower():
                            continue
                        # kamu bisa tambah pattern lain kalau selain SPLN
                        if "spln" in s.lower():
                            if s not in out:
                                out.append(s)
                    return out
...
def parse_sections_from_main_table(df) -> Tuple[Dict[str, List[Dict]], Optional[str]]:
    """
    Parse kasar dulu, nanti kita rapikan di postprocess agar sesuai Excel.
    """
    ...
    # kolom type/qty (kadang kebaca benar, kadang tidak)
    type_c = find_col_idx(df, ["raw material", "type", "material"])
    qty_c = find_col_idx(df, ["quantity"])

    if type_c is None or qty_c is None:
        # fallback 2 kolom paling kanan
        if df.shape[1] >= 2:
            type_c = df.shape[1] - 2
            qty_c = df.shape[1] - 1

    # specified kita ambil dari range (unit+1 ... type-1)
    current = None
    start_r = 1
    ...
    for r in range(start_r, df.shape[0]):
        ...
        # marking line (kadang nyasar ke tabel)
        ...
        # specified = gabung kolom antara unit dan type
        specified_parts = []
        ...
        # kalau specified ternyata berisi marking, pindahin ke marking
...
# ----------------------------
# Postprocess agar sesuai Excel kamu
# ----------------------------
def repair_conductor_desc_merge_layers(cond_items: List[Dict]) -> List[Dict]:
    """
    Gabungkan:
      "OD/LP/DL of outer layer"
      "1st Layer 1 + 6"
      "2nd Layer 12"
    menjadi 2 baris:
      "OD/LP/DL of outer layer 1st Layer 1 + 6"
      "OD/LP/DL of outer layer 2nd Layer 12"
    """
    out = []
    base = None
    for it in cond_items:
        ...
        if d.lower() == "od/lp/dl of outer layer":
            ...
            # jangan masukin base sebagai baris sendiri
            continue
...
def apply_type_qty_from_raw_cell(it: Dict) -> Dict:
    """
    Normalisasi raw material:
    - kalau raw_type_cell berisi "PE Insulation 128.27" -> type="PE Insulation", qty="128.27"
    - kalau raw_qty_cell juga gabung -> pakai yang ada angka
    """
    it = dict(it)
    ...
    # kandidat gabungan paling sering ada di rt, tapi kadang di rq
    candidates = [rt, rq]
    best_t = None
    best_q = None
    ...
    # kalau tidak ada pasangan, coba ambil text doang
    if best_t is None:
        if rt:
            best_t = rt
        elif rq:
...
def fix_spec_suffix_SZ(cond_items: List[Dict]) -> List[Dict]:
    """
    Kasus kamu:
      Conductor Specified: "6.75 / 68 - 108 /" lalu ada "S" nyasar
      Conductor Specified: "11.25 / 113 - 158 /" lalu ada "Z" nyasar
    Biasanya S/Z kebaca di raw_type_cell sebagai "S" atau "Z".
    """
    out = []
    for it in cond_items:
        it = dict(it)
        ...
        rt = norm(it.get("raw_type_cell"))
        # jika rt cuma S/Z dan spec berakhir '/', tempelkan
...
def normalize_marking(marking: Optional[str]) -> Optional[str]:
    if not marking:
        return None
    # hapus label kalau ikut kebawa
    m = re.sub(r"(?i).*Marking of Cable by Roll Printing\s*:\s*", "", marking).strip()
    return norm(m)


def postprocess_sections(header: Dict, sections: Dict[str, List[Dict]], marking: Optional[str]) -> Tuple[Dict, Dict, Optional[str]]:
    # MARKING: ambil dari pdf text kalau ada
    ...
    # ambil material conductor dari salah satu row yang punya "Aluminium Alloy Wire 207.3"
    conductor_type = None
    conductor_qty = None
    for it in cond:
        ...
        t = norm(it2.get("type"))
        q = norm(it2.get("qty"))
        if t and q and ("wire" in t.lower()):  # heuristik
            conductor_type = t
            conductor_qty = q
            break

    # set type/qty conductor untuk SEMUA row
    fixed_cond = []
    for it in cond:
        it = dict(it)
    ...
    # ekstrak material list (urutan muncul)
    ...
    # bersihkan: jangan sampai marking nyasar di specified
    fixed_cov = []
    for it in cov:
        it = dict(it)
        it["desc"] = clean_desc(it.get("desc") or "")
    ...
    # assign material covering ke baris 1..N (sesuai Excel kamu)
    ...
    for it in pk:
        ...
        it["qty"] = None
        fixed_pk.append(it)
    # assign material packing ke baris 1..N (seperti Excel: Wooden Drum 120 -> row1, End Cap -> row2)
    ...
    # Ref.Spec: buang yang bukan SPLN
...
def parse_tds(pdf_path: str, debug: bool = False):
    ...
    # marking final: page text lebih dipercaya
    marking_txt = extract_marking_from_page_text(pdf_path)
    marking = marking_txt or marking_tbl

    header, sections, marking = postprocess_sections(header, sections, marking)
...
def build_excel_like_rows(header: Dict, sections: Dict[str, List[Dict]], marking: Optional[str]) -> List[Dict]:
    ...
    for i in range(N):
        ...
        # Ref.Spec: jangan pernah isi "RAW MATERIAL"
            ...
            conn.execute(
                text("""
                    INSERT INTO tds_rows (
                      ...
                      `Packing`, `Packing Unit`, `Packing Specified`, `Packing Type`, `Quantity`
                    ) VALUES (
                      :document_id, :row_no,
                      ...
                      :Packing, :Packing_Unit, :Packing_Specified, :Packing_Type, :Quantity
                    )
                """),
                {
                    "document_id": doc_id,
