...
# =========================================================
# PARSER (COPIED FROM YOUR SUCCESSFUL FILE)
# =========================================================
...
def extract_packing_pairs_from_text(pdf_path: str, debug: bool = False) -> List[Tuple[str, str]]:
    try:
        with pdfplumber.open(pdf_path) as pdf:
    ...
    text_all = text_all_raw.replace("\r", "\n")
    m = re.search(
        r"\b4\s+Packing\b(.*?)(?:\bPrepared by\b|\bDESIGN CHANGE NOTICE\b|\Z)",
        text_all,
        flags=re.I | re.S,
    )
    block = m.group(1) if m else text_all
    block_1sp = re.sub(r"\s+", " ", block).strip()

    wd = re.search(r"Wooden\s+Drum\s+(\d+)\s+(\d+)", block_1sp, flags=re.I)
    ec = re.search(r"End\s+Cap\s+(\d+)", block_1sp, flags=re.I)
    ...
    if not any("wooden drum" in t.lower() for t, _ in out):
        if re.search(r"Wooden\s+Drum", block_1sp, flags=re.I):
            msize = re.search(r"Wooden\s+Drum\s+(\d+)", block_1sp, flags=re.I)
            size = msize.group(1) if msize else "120"
            qty = "1"
            if msize:
                after = block_1sp[msize.end():]
                mqty = re.search(r"\b([1-9])\b", after)
                if mqty:
                    qty = mqty.group(1)
            out.insert(0, (f"Wooden Drum {size}", qty))
    ...
    if debug:
        print("[DEBUG] packing pairs:", ordered)
...
def parse_sections_from_main_table(df) -> Tuple[Dict[str, List[Dict]], Optional[str]]:
    ...
    for r in range(start_r, df.shape[0]):
        ...
        # Final Test first row fix
        if not desc:
            for cc in range(df.shape[1]):
                v = clean_desc(norm(df.iat[r, cc]))
                if v and v.lower() not in ["unit", "specified", "type", "quantity", "description"]:
                    desc = v
                    break
...
def postprocess_sections(header: Dict, sections: Dict[str, List[Dict]], marking: Optional[str], pdf_path: str, debug: bool = False) -> Tuple[Dict, Dict, Optional[str]]:
    ...
    # Packing ALWAYS from text
    pk = sections.get("Packing", [])
    pk_m2 = extract_packing_pairs_from_text(pdf_path, debug=debug)
    ...
    # assign only first 2 rows
    for i in range(len(fixed_pk)):
        fixed_pk[i]["type"] = None
        fixed_pk[i]["qty"] = None

    for i, (t, q) in enumerate(pk_m2[:2]):
        if i < len(fixed_pk):
            fixed_pk[i]["type"] = t
            fixed_pk[i]["qty"] = q
    ...
    # safety: jangan ada marking string di Covering Specified
    for it in sections.get("Covering", []):
        if it.get("specified") and is_marking_text(it["specified"]):
            it["specified"] = None

    # safety: jangan ada marking nyasar di Covering (desc maupun specified)
    cleaned = []
    for it in sections.get("Covering", []):
        d = it.get("desc") or ""
        s = it.get("specified") or ""

        if is_marking_text(d) or is_marking_text(s):
            # buang row marking ini dari covering
            continue

        # kalau ada potongan marking kebawa
        if d and is_marking_text(d):
            it["desc"] = None
        if s and is_marking_text(s):
            it["specified"] = None

        cleaned.append(it)

    sections["Covering"] = cleaned
...
def parse_tds(pdf_path: str, debug: bool = False):
    ...
    header, sections, marking = postprocess_sections(header, sections, marking, pdf_path, debug=debug)
    return header, sections, marking
...
# =========================================================
# ONE FILE = ONE TABLE (based on filename)
# =========================================================

def safe_base_from_filename(pdf_path: str) -> str:
...
def make_candidate_table_name(pdf_path: str) -> str:
    base = safe_base_from_filename(pdf_path)
    # tanpa prefix
    max_len = 64
    if len(base) > max_len:
...
def get_unique_table_name(conn, base_name: str) -> str:
    """
    Jika nama tabel sudah ada, buat tds_xxx_2, tds_xxx_3, dst
    """
    name = base_name
    i = 1
    while True:
        exists = conn.execute(text("SHOW TABLES LIKE :t"), {"t": name}).fetchone()
        if not exists:
            return name
        i += 1
        suffix = f"_{i}"
        cut = 64 - len(suffix)
        name = (base_name[:cut] + suffix) if len(base_name) > cut else (base_name + suffix)


def ensure_db_and_index(mysql_server_url: str, db_name: str):
    if not re.fullmatch(r"[A-Za-z0-9_]+", db_name):
        raise ValueError("db_name hanya boleh huruf/angka/underscore")
...
def ensure_perfile_table(conn, table_name: str):
    conn.execute(text(f"""
...
def upsert_doc_index(conn, file_path: str, file_hash: str, table_name: str, parser_name: str):
    conn.execute(
        text("""
            INSERT INTO tds_documents (file_path, file_hash, table_name, parser_name)
            VALUES (:p,:h,:t,:pn)
        ...
        """),
        {"p": file_path, "h": file_hash, "t": table_name, "pn": parser_name}
    )


def insert_rows_into_table(conn, table_name: str, rows: List[Dict]):
...
def main():
    ...
    engine = ensure_db_and_index(args.mysql_server, args.db_name)
    ...
    for pdf_path in iter_pdfs(args.pdf_dir):
        ...
        try:
            ...
            # Parse with your proven parser
            header, sections, marking = parse_tds(pdf_path, debug=args.debug)
            if not sections:
                raise RuntimeError("tabel utama tidak kebaca")

            rows = build_excel_like_rows(header, sections, marking)

            with engine.begin() as conn:
                base_table = make_candidate_table_name(pdf_path)
                # if this hash already exists, reuse table name from index
                existing = conn.execute(
                    text("SELECT table_name FROM tds_documents WHERE file_hash=:h"),
                    {"h": fh}
                ).fetchone()
                if existing:
                    table_name = existing[0]
                else:
                    table_name = get_unique_table_name(conn, base_table)

                ensure_perfile_table(conn, table_name)
                insert_rows_into_table(conn, table_name, rows)
                upsert_doc_index(conn, pdf_path, fh, table_name, "onefile_onetable_filename_v1")

            ok += 1
            print(f"[OK] {os.path.basename(pdf_path)} -> {table_name} rows={len(rows)}")
    ...
    # ============================
# API untuk dipanggil dari app.py
# ============================
_ENGINE_CACHE = {}

def process_one_pdf(pdf_path: str, mysql_server: str, db_name: str, debug: bool = False) -> str:
    """
    Proses 1 PDF -> 1 tabel.
    Return: table_name yang dipakai.
    """
    # cache engine biar cepat untuk banyak klik / banyak file
    key = (mysql_server, db_name)
    if key not in _ENGINE_CACHE:
        _ENGINE_CACHE[key] = ensure_db_and_index(mysql_server, db_name)
    engine = _ENGINE_CACHE[key]

    fh = sha256_file(pdf_path)

    # parse pakai parser yang sudah sukses
    header, sections, marking = parse_tds(pdf_path, debug=debug)
    if not sections:
        raise RuntimeError("tabel utama tidak kebaca")
